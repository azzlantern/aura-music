...
const getLinePosSpring = (relativeIndex: number): SpringConfig => {
  ...
  // "Previous line, current line... scrolling speed needs to be faster"
  // Stiffness > 600 feels almost instant.
  if (relativeIndex <= 0) {
    return { mass: 1, stiffness: 600, damping: 60, precision: 0.1 };
  }
  ...
  if (dist > 8) {
    return { mass: 1, stiffness: 40, damping: 18, precision: 0.1 };
  }

  // Exponential Decay for Large Variation
  // Base stiffness 450 (Start fast)
  // Factor 0.45 (Decay fast)
  // 1: 202
  // 2: 91
  // 3: 41
  // This creates a very distinct "accordion" separate effect.
  const base = 350;
  const stiffness = Math.max(40, base * Math.pow(0.45, dist));

  // Over-damping to prevent bounce when they arrive
  const damping = Math.sqrt(stiffness) * 3.5;
...
const LyricLine = React.memo(
  ({
    ...
    const springSystem = useRef(
      new SpringSystem({ scale: 1, posY: 0 }),
    ).current;
    ...
    useLayoutEffect(() => {
      ...
      const loop = (now: number) => {
        ...
        // If visualState is true (user dragging), we want a stiff, responsive spring (1:1 feel) for EVERYTHING.
        // If auto-scrolling, we use the variable physics to create the "active fast, future slow" effect.
        if (metrics.visualState) {
          springSystem.setTarget("posY", metrics.targetPosY, {
            mass: 0.5,
            stiffness: 400,
            damping: 35,
          });
        ...
        let normDist = 1;
        let targetScale = 1; // Default future scale

        if (divRef.current) {
          const lineTop = divRef.current.offsetTop;
          const lineCenter = lineTop + divRef.current.offsetHeight / 2;

          // Calculate distance using current physical position (currentScrollY)
          const currentScrollY = springSystem.getCurrent("posY");

          // Calculate absolute visual distance from the "Active Focal Point"
          // activePoint is where the center of the active line SHOULD be on the screen (in absolute doc coords)
          // lineCenter is where this specific line is in absolute doc coords.
          // We adjust lineCenter by the scroll difference to get its screen position relative to the active point?
          // Actually simpler: The distance that matters for scale is how far this line is from the "Active Highlight Zone".
          // The "Active Highlight Zone" corresponds to `metrics.activePoint` in the document coordinate space.

          const dist = Math.abs(lineCenter - metrics.activePoint);

          // Scale Logic
          const range = 450; // Extended range for smoother falloff
          normDist = Math.min(dist, range) / range; // 0 to 1

          if (isActive) {
            targetScale = 1; // No magnification
          }

          // Minimum scale
          //targetScale = Math.max(0.85, targetScale);
        ...
        if (divRef.current) {
          ...
          // Opacity Logic
          const minOpacity = metrics.visualState ? 0.5 : 0.3;
          const opacityCurve = 1.0 - Math.pow(normDist, 0.5);
        ...
        if (line.words && line.words.length > 0) {
          line.words.forEach((word, i) => {
            ...
            if (currentTime < word.startTime) {
              ...
              span.style.transform = "translate3d(0,-2px,0) scale(1)";
              span.style.textShadow = "";
            } else {
    ...
    return (
      <div
        ...
        className={`
                  py-3 rounded-xl cursor-pointer mr-6 px-6
                  origin-left
      ...
      >
        ...
        <div
          className={`${textSizeClass} font-bold leading-tight text-white tracking-tight`}
        >
